...
namespace agents {
  namespace middleware {
    ...
    struct CommandsProcessor {
      ...
      explicit CommandsProcessor(const MessageDictionary<AgentMessage>& agent_message_dictionay) :
    ...
    private:
      void ProcessTranformedMessage(const NanoMessageType& nano_message_type) {
        const auto command_to_execute = agent_message_dictionay_.GetMessageFromDictionary(nano_message_type.id);

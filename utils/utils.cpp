...
namespace agents {
  namespace utils {
    ...
    bool PackMessageToString(const std::string_view input_message, PacketBufferIterator packet_message_iterator,
      ...
      std::memcpy(packet_message_iterator + MESSAGE_SIZE_DEFAULT, &message_type_pack, MESSAGE_TYPE_SIZE);
      // pack message to buffer
      std::memcpy(
        (packet_message_iterator + MESSAGE_SIZE_DEFAULT + MESSAGE_TYPE_SIZE), input_message.data(), message_size);
      result = true;
      return result;
    }
    ...
    std::optional<std::uint32_t> GetPackectMessageSize(PacketBufferIterator packet_message_iterator) {
      ...
      std::memcpy(&enconded_message_size, packet_message_iterator, MESSAGE_SIZE_DEFAULT);

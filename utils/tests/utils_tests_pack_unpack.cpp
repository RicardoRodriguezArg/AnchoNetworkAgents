...
TEST_F(UtilsPackUnpackMessage, GivenValidInputUnpackIsOk) {
  // Given valid input
  const auto& expected_input_message = CreateInputString(InputStringCase::kValid4BytesInputString);
  EXPECT_EQ(15U, expected_input_message.size());
  const auto result =
    agents::utils::PackMessageToString(expected_input_message, raw_buffer_.begin(), agents::common::MessageType::EVENT);
  ASSERT_TRUE(result);
  ...
  const auto message_type_opt =
    agents::utils::GetPackectMessageType(raw_buffer_.begin() + agents::utils::MESSAGE_SIZE_DEFAULT);
  ASSERT_TRUE(message_type_opt.has_value());
  const auto x = message_type_opt.value();
  EXPECT_EQ(static_cast<common::MessageType>(message_type_opt.value()), common::MessageType::EVENT);
  ...
  const auto encoded_data = agents::utils::GetPackectMessageData(
    raw_buffer_.begin() + agents::utils::MESSAGE_SIZE_DEFAULT + agents::utils::MESSAGE_TYPE_SIZE, enconded_data_size);
...
TEST_F(UtilsPackUnpackMessage, GivenInValidInputUnpackNotEncodec) {
  // Given valid input
  CleanInputBuffer();
  const auto& expected_input_message = CreateInputString(InputStringCase::kEmptyInputString);
  ASSERT_TRUE(expected_input_message.empty());
  const auto result =
    agents::utils::PackMessageToString(expected_input_message, raw_buffer_.begin(), common::MessageType::EVENT);
  ASSERT_FALSE(result);
}

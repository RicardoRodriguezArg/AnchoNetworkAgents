...
public:
  ...
  void CleanInputBuffer() {
    std::fill_n(raw_message_copy.begin(), agents::utils::MAX_BUFFER_SIZE, 0);
  ...
  void Start() {
    ...
    while (is_server_operating_) {
      const auto number_of_bytes =
          udp_socket_->ReadFromAllClients(buffer_.begin(),
      ...
      if (message_type_opt.has_value()) {
        std::string raw_message_copy{buffer_.begin(),
                                     buffer_.begin() + number_of_bytes};
        message_handler_.HandleMessage(message_type_opt.value(),
                                       raw_message_copy);
      }
    }
  }

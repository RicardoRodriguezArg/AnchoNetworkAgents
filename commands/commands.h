...
namespace agents {
  namespace middleware {
    namespace commands {
      ...
      struct StopDevice {
        explicit StopDevice(std::shared_ptr<ProxyManagerType> device_manager) :
        ...
        State Execute(std::uint32_t device_id) {
          ...
          if (device_opt.has_value()) {
            ...
            const auto command_execution = device_proxy.ExecuteCommand(
              static_cast<std::uint32_t>(Id::kStopDevice));
            // TODO: Add ACK component, as we always we want to receive ACK from
            // device
            command_execution_result =
              (command_execution > 0) ? State::kExecuted : State::kFailed;
          }
      ...
      struct RequestCurrentStatus {
        explicit RequestCurrentStatus(
          std::shared_ptr<ProxyManagerType> device_manager) :
            device_manager_(device_manager) {}
      ...
      struct RequestStatusOfAllDevices {
        explicit RequestStatusOfAllDevices(
          std::shared_ptr<ProxyManagerType> device_manager) :
            device_manager_(device_manager) {}
      ...
      //
      struct ResetDevice {
        explicit ResetDevice(std::shared_ptr<ProxyManagerType> device_manager) :
            device_manager_(device_manager) {}
      ...
      //
      struct StartDevice {
        explicit StartDevice(std::shared_ptr<ProxyManagerType> device_manager) :
            device_manager_(device_manager) {}

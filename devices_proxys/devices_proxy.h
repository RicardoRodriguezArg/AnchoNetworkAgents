...
namespace agents {
  namespace middleware {
    namespace proxys {
      enum class State : std::uint8_t { INIT, NON_INIT, NON_CONNECTED, CONNECTED, LAGGING };

      using CommandMessageDictionary = MessageDictionary<::agent_interface_CommandWithArguments>;

      struct DeviceProxy {

        explicit DeviceProxy(std::function<bool(const std::string&)> command_dispatcher_callback,
        ...
        std::int32_t ExecuteCommand(std::uint32_t command_id_to_execute) {
          ...
          const auto nano_commnand_opt = message_command_dict_->GetMessageFromDictionary(command_id_to_execute);
          if (nano_commnand_opt) {
            // TODO:Change to use string view and avoid copy
            const auto encoded_command = data_encoder_.EncodeCommand(*nano_commnand_opt);
